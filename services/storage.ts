...
/**
 * 字符串规范化，用于缓存键的匹配
 * 移除非字母数字、非空格和非中文字符，并将多个空格合并为一个
 */
export const normalizeQuery = (q: string): string => {
  return q.trim()
    .toLowerCase()
    .replace(/[^\w\s\u4e00-\u9fa5]/gi, "") // 仅保留中文字符、字母、数字和空格
...
export const saveAdviceToCache = (advice: string, phase: CyclePhase, query?: string) => {
  ...
  if (!query) {
    ...
    const cacheData = localStorage.getItem(QUERY_CACHE_KEY);
    let cache: Record<string, QueryCacheEntry> = cacheData ? JSON.parse(cacheData) : {};
    const key = normalizeQuery(query);
    const now = Date.now();
